---
title: 'BEGINNERS Social Networks Course: Session 1'
author: "Matthew Silk"
date: "2025-10-20"
output: 
  html_document:
...
Again, it is easy to compare the matrix we produce here with the one in the powerpoint slides.
...
1) an adjacency matrix to an edge list
and
2) an incidence matrix to an adjacency matrix

You can see an example of each by revealing the code chunk below.
...
The most frequently used additional information in animal social networks are weighted edges. We will explore various aspects of weighted networks, and handling and plotting them with igraph here.
...
#If we plot these networks then (even excluding information on edge weights) we can see that the network has clear
#social groups in the first part of the study but not in the second.
...
And that is all for the end of the first session - hopefully you are now feeling more confident about the different ways to represent social network data, how to handle social network datasets in R and how to generate some nice network plots.

Next session we will start exploring social network analysis.
